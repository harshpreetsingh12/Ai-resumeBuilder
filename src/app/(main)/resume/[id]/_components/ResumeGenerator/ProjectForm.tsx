...
const ProjectForm = ({project,position}:ProjectCompProps) => { 
  ...
  return (
    <div className="w-full mx-auto p-6 bg-white shadow-md border-gray-300 border-[1px] rounded-md text-sm mb-4 relative">
      ...
      <span className="absolute top-6 right-6 cursor-pointer" onClick={removeProject} >
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className='flex flex-wrap gap-2 py-2 mb-4'>
          {watch('skillsUsed').map((skill,index)=>{
              return (
                <div key={index} className='flex items-center gap-2 rounded-lg bg-blue-600 text-white  p-1 px-2 text-xxs '>
                  <span >
                    {skill}
                  </span>
